...
4. **Register every item** in `data/catalog.json` with `id`, `type`, `name`, `description`, `tags`, `category`, `thumbnail`, `path`, `featured`, `date` (optional: `projectId`, `related`, `meta`).
...
7. **One complete deliverable per reference URL** unless the user asks otherwise. Finish registration + audit + verify in the same turn when possible.
